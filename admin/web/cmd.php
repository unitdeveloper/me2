...
$loadIp = fetch([
    'host' => 'https://ipinfo.io/'.$ip.'?token=4982574f9fa9e7',
    'method' => 'GET',
    'header' => [
...
if (!@$_GET['list']){
    ...
    // foreach ($res->iptable->deny as $key => $value) {
    //     if ($value->ip == $ip ){
    ...
    $line = fetch([
        'host' => 'http://www.ewinl.com/index.php?r=line-bot/robot&token=4573&ip='.$ip.'&message='.json_encode($loadIp).'&url=cmd.php',
        'method' => 'POST',
        'header' => [
            'Content-type: application/json',
        ]
