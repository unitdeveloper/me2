app.controller('purchaseOrder', ['$scope', '$http', '$log',
function($scope,$http,$attrs) {
    ...
    $scope.PurchaseReceive = '';
    $http.get('index.php?r=Purchase/order/get-receive&id='+$('form#form-purchase-order').data('key'))
    .then(function (response) {
        $scope.PurchaseReceive   = response.data;
    });
        ...
        if (confirm('Are you sure you want to delete this?')) {
            ...
            angular.forEach($scope.PurchaseLine, function(selected){
                ...
                if(selected.selected){
                    ...
                    $http.post('index.php?r=Purchase/order/ajax-delete-pur-line',data).then(function(data){
                        console.log('Remove');
                    })
                }
            });
  ...
  $scope.$watch("searchProduct",function(newValue,oldValue){
    ...
    if(newValue.length > 2){
      ...
      $http({
        method:'GET',
        url:'index.php?r=items/ajax/find-items-json&limit=20&word='+newValue,
        data:{word:newValue},
