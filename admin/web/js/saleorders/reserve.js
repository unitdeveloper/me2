...
let renders = (data, div) => {
  ...
  data.length > 0
    ? data.map(model => {
        html += `<tr 
                    data-key="${model.id}" 
                    data-address="${model.address.address}" 
                    class="${model.head === 1 ? "bg-info" : ""}" 
                    data-term="${model.term}"
                    data-vatPercent="${model.vat_percent}" 
                    data-incVat="${model.include_vat}">
...
$("body").on("click", "button.selected-customer", function() {
  ...
  let vatPercent  = parseInt($(this).closest("tr").attr("data-vatpercent"));
  let incVat      = parseInt($(this).closest("tr").attr("data-incvat"));
  ...
  fetch("?r=SaleOrders/reserve/load-data", {
    method: "POST",
    body: JSON.stringify({
      line: JSON.parse(sessionStorage.getItem("reserve-data")),
      headers: headers
    }),
    headers: {
      "Content-Type": "application/json",
    ...
    .then(response => {
      ...
      localStorage.setItem("reserve-new-sale-line", JSON.stringify(response.item));
    })
    .catch(error => {
      console.log(error);
    });
...
$("body").on("click", "button.next-to-finish", function() {
  ...
  if($('input[name="ship_date"]').val() == ""){   
    ...
    soundClick2.play();
    let headers = localStorage.getItem("reserve-order")
                    ? JSON.parse(localStorage.getItem("reserve-order"))
                    : [];
    //Finish  
    fetch("?r=SaleOrders/reserve/finished", {
      method: "POST",
      body: JSON.stringify({ ship_date: headers.ship_date, order_id: headers.order_id }),
      headers: {
        "Content-Type": "application/json",
        "X-CSRF-Token": $('meta[name="csrf-token"]').attr("content")
      }
      ...
      .then(response => {
        if (response.status === 200) {
          ...
          localStorage.removeItem("reserve-new-sale-line");
          localStorage.removeItem("sale-header");
          localStorage.removeItem("session");
          sessionStorage.removeItem("reserve-data");
...
$('body').on('click', 'input[name="produce"]', function(){
  ...
  let data    = JSON.parse(localStorage.getItem("reserve-new-sale-line"));
  let update  = data.map((model, key) => {
                  return key === el ? Object.assign({}, model, { bom: 1 }) : model;
              });
  localStorage.setItem("reserve-new-sale-line", JSON.stringify(update));
...
$("body").on("click", "button.create-sale-line", function() {
  ...
  $('body').find('input[name="produce"]').each(function() {
  ...
  if ((headers.header.date === "") || (!headers.customer.term)) {
    ...
    fetch("?r=SaleOrders/reserve/create-sale-line", {
      method: "POST",
      ...
      $(el)
        ...
        .map((i, p) => {
          ...
          if (str.search("єѬјзҕѥѝѧьзҖѥ") > -1) {
            ...
            // บริษัท สยามโกลบอลเฮ้าส์ จํากัด (มหาชน) สํานักงานใหญ่ GOBAL
            $(p).nextAll().slice(2, 3).css({
                "background-color": "rgb(0, 224, 247)",
                padding: "5px !important"
              });
            total = Number(
              $(p).nextAll().slice(2, 3).text().replace(/[^0-9\.-]+/g, "")
            );
            endpage = $(p).closest("div").index();
            if(endpage > 2){ 
              endpage = (endpage -1)   
            ...
            total = Number(
              $(p).prevAll().slice(0, 1).text().replace(/[^0-9\.-]+/g, "")
            );
            endpage = $(p).closest("div").index();
            if(endpage > 2){ endpage = endpage - 1 }
          } else if (str.search("รวมราคาสินค้าไม่รวมภาษีมูลค่าเพิ�ม") > -1) {
...
$(document).ready(function() {
  ...
  if (data.length > 0) {
    ...
    let orders = localStorage.getItem("reserve-order")
      ? JSON.parse(localStorage.getItem("reserve-order"))
      : [];
    ...
    findCompany(res => {
                ...
                switch (store) {
                      ...
                      if(rows.search('%') > -1){
                        ...
                        if(price > 0){
                          ...
                          if(homehub.position_qty_num == 'prevAll'){
                            $(p).prevAll().slice(homehub.position_qty, (homehub.position_qty + 1)).css("background", "red");
                          }else{
                            $(p).nextAll().slice(homehub.position_qty, (homehub.position_qty - 1)).css("background", "red");
                          }

                          if(homehub.position_discount_num == 'prevAll'){
                            $(p).prevAll().slice(homehub.position_discount, (homehub.position_discount + 1)).css("background", "orange");
                           // $(p).prevAll().slice(3, 4).css("background", "orange");
                          }else{
                            $(p).nextAll().slice(homehub.position_discount, (homehub.position_discount - 1)).css("background", "orange");
                            //$(p).prevAll().slice(3, 4).css("background", "orange");
                          }
                        ...
                        //$(p).nextAll().slice(1, 2).css("background", "red");
                        //$(p).nextAll().slice(2, 3).css("background", "orange");
                        //$(p).nextAll().slice(0, 1).css("background", "green");

                        //qty     = Number($(p).nextAll().slice(2, 3).text().replace(/[^0-9\.-]+/g, "")); // red
                        //sumline = Number($(p).nextAll().slice(0, 1).text().replace(/[^0-9\.-]+/g, "")); // green
                        discount= 0;                        
                        price   = sumline / qty;
                  ...
                  case 5: // HOME PRO
                    ...
                    if (str.search("/1 EA") > -1) {
                      ...
                      // ถ้าเจอเครื่องหมาย % ในตัวเลขแสดงว่าเป็นเปอร์เซ็นส่วนลด
                      //console.log(rows.indexOf('%'));
                      if(rows.indexOf('%')){

                        $(p).prevAll().slice(1, 2).css("background", "#51ff00");
                        $(p).nextAll().slice(1, 2).css("background", "purple");
                        ...
                        sumline = Number(rows.replace(/[^0-9\.-]+/g, ""));
                        price   = sumline / qty;
      ...
      sessionStorage.setItem("reserve-data", JSON.stringify(data));
...
let renderTable = data => {
        ...
        if (model.status) {
          html += `
                    <tr data-key="${model.id}" data-row="${key}">
                        ...
                        <td class="text-right">${number_format(total.toFixed(2))}</td>
  ...
  html += `
                <tr class="bg-gray" data-row="0">
                    ...
                    <td class="text-center"><input type="checkbox" name="add-produce" value="" /></td>
  ...
  data.length > 0
    ? data.map(model => {
       ...
       // let total = Number((model.qty * model.price).toFixed(2));
        let uitPrice = model.price;
        sum += total;
        html += `
                <tr class="">
                    <td class="bg-gray">${i}</td>
                    <td class="text-left">${model.code}</td>
                    <td>${model.name}</td>
                    <td class="text-right">${model.qty}</td>
                    <td class="text-right">${number_format(uitPrice.toFixed(2))}</td>
    ...
    if (search.trim()) {
      findItems(search, res => {
        if (res.status === 200) {
          ...
          $('body').find('input[name="add-produce"]').val(res.id).attr('checked', res.items[0].make);
...
$("body").on("keypress", 'input[name="add-totalprice"]', function(e) {
  ...
  let oldPrice = $('input[name="add-price"]').attr('old-val') * 1;
  let newPrice = $('input[name="add-price"]').val() * 1;
  if(oldPrice === newPrice){ // ถ้าเปลี่ยนจำนวน ให้ใช้จำนวนจาก total มาคำนวณ (เพื่อหาส่วนลด)
    // total       = Number(Number($('input[name="add-totalprice"]').val()) / Number($('input[name="add-qty"]').val()));
  }else{
    // total       = $('input[name="add-qty"]').val() * $('input[name="add-price"]').val();
  }
  ...
  let newData = {
    ...
    make: $('body').find('input[name="add-produce"]').is(':checked')
  };
...
$("body").on("click", "button.delete-line", function() {
  ...
  let deleteLine = id => {
    ...
    localStorage.setItem("reserve-new-sale-line", JSON.stringify(line));
    renderTable(line);
