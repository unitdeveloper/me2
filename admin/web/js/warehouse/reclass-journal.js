...
$(document).ready(function () {
    ...
    setTimeout(function () {
        route('warehousemoving/reclass-journal/ajax-line&id=' + $(form + 'documentno').attr('data'), 'GET', { data: false }, 'ew-adj-line');
...
$('body').on('change',form+'typeofdocument',function(){
	...
	ValidateSeries('Reclass','5','item_reclass_journal','DocumentNo','all',form+'documentno',true);
});
...
function validateLocation() {
    ...
    var location = [];
    $('#Item_Adjust_Line').find('tr.data').each(function (i, el) {
        var key = $(el).attr('data-key');
        var locationSort = $(el).find('#itemreclassjournalline-location-' + key + '').val();
        var locationDest = $(el).find('#itemreclassjournalline-to_location-' + key + '').val();
        if (locationSort === locationDest) {
            $(el).find('i.fa-arrow-right').removeClass('text-green').addClass('blink').addClass('text-red');
            $(el).find('#itemreclassjournalline-to_location-' + key + '').attr('style', 'border:1px dotted red;').focus();
            location.push(false);
        } else {
            $(el).find('#itemreclassjournalline-to_location-' + key + '').attr('style', 'border:1px solid green;');
            $(el).find('i.fa-arrow-right').removeClass('blink').removeClass('text-red').addClass('text-green');
            location.push(true);
        }
    });
    return allTrue(location);
}
...
$('body').on('click', '#ew-Post-Adjust', function () {
    if (confirm('Confirm POST ! ')) {
        if (validateLocation()) {
            var $data = {
                id: btoa($(form + 'documentno').attr('data')),
                key: $('input[name="eWin-backend"]').val(),
                postDate: $(form + 'postingdate').val(),
            };
            $.ajax({
                url: "index.php?r=warehousemoving/reclass-journal/post-journal&id=" + btoa($(form + 'documentno').attr('data')),
                type: "POST",
                data: $data,
                async: true,
                dataType: 'JSON',
            ...
            $.notify({
                ...
                message: 'คลังสินค้า ต้องไม่เหมือนกัน',
            }, {
                    // settings
                    type: 'warning',
                    delay: 1500,
    ...
    if ($('.items:checked').serialize() != '') {
        ...
        $('tr').each(function (i, el) {
            ...
            if (iCheck.is(':checked')) {
                ...
                }, '#Item_Adjust_Line', 'warehousemoving/reclass-journal');
            }
        });
            ...
            success: function (getData) {
                ...
                if (obj.all == 1) {
                    AddItemToLine({
                        ...
                        price: obj.std,
                        id: $(form + 'documentno').attr('data'),
                        no: $(form + 'documentno').attr('ew-no_'),
        ...
        success: function (obj) {
            if (obj.status == 200) {
                ...
                var row = $('<tr data-key="' + obj.id + '">' +
                    ...
                    '<td align="right"><input type="number" class="form-control text-right journal-qty" name="ItemReclassJournalLine[Quantity][' + obj.id + ']" value="' + Math.abs(obj.qty) + '" style="width:90px;"></td>' +
                    '<td class="text-right">' +
                    '<select id="ItemReclassJournalLine-unit_of_measure-' + obj.id + '" class="form-control option-change" field-name="unit_of_measure" name="ItemReclassJournalLine[unit_of_measure][' + obj.id + ']">' + MeasureOptions + '</select>' +
                    '</td>' +
                    '<td class="text-right hidden journal-qty-after">' + Number(obj.remain + obj.qty).toFixed(2) + '</td>' +
                    '<td class="text-right hidden">' +
                    '<input type="number" class="form-control text-right" name="ItemReclassJournalLine[unit_price][' + obj.id + ']" value="' + Math.abs(obj.price) + '" style="width:120px;">' +
                    '</td>' +
                    ...
                    '<td><select name="ItemReclassJournalLine[location][' + obj.id + ']" class="form-control option-change" field-name="location">' + options + '</select></td>' +
                    '<td class="text-center" style="padding-top:15px;"><i class="fas fa-arrow-right text-red blink"></i></td>' +
                    '<td><select name="ItemReclassJournalLine[to_location][' + obj.id + ']" class="form-control option-change" field-name="to_location">' + options + '</select></td>' +
  ...
  function EditText($n,e,){
    ...
    $('body').on('keydown','input[id="ew-text-editor"]',function(e){
        ...
        if ((e.which == 13) || (e.which == 9)) {
            var $doc    = $(form+'documentno').attr('data');
            ...
            $.ajax({
                url: "index.php?r=warehousemoving/reclass-journal/ajax-line&id="+$doc,
                type: "POST",
                data: $data,
                success: function(getData) {
    ...
    $('body').on('change','.option-change',function(e){
...
if (confirm('Do you want to delete  ?')) {
    ...
    tr.fadeOut(500, function(){
        tr.remove();
        route('index.php?r=warehousemoving/reclass-journal/delete-adj-line','POST',data,'Navi-Title');
...
$('body').on('click','button.submit-form',function () {
    $('form#item-reclass-journal').submit();
});
...
$('body').on('change','.locations',function(){
    ...
    $.ajax({
        url:'index.php?r=warehousemoving/reclass-journal/get-inventory-by-location',
        type:'POST',
