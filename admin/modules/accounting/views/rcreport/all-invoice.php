...
const getDataFromAPI = (obj,callback) => {
    fetch("?r=accounting/rcreport/all-invoice-ajax", {
        method: "POST",
...
const renderTable = (res) => {
    ...
    let html = `<div class="table-responsive">
                <table class="table table-bordered" id="export_table">
                    <thead>
                        ...
                        <tr class="bg-gray">
                            ...
                            <th>{$Yii::t('common','Amount')}</th>
                            <th>{$Yii::t('common','Vat')}</th>
                            <th>{$Yii::t('common','Total')}</th>                             
                            <th>{$Yii::t('common','Sale Order')}</th>
                            <th>{$Yii::t('common','Sale Balance')}</th>
                            <th class="text-right" title="เลขที่บัญชีต้นทาง">{$Yii::t('common','No')}</th>
                            <th class="text-right">{$Yii::t('common','Payment')}</th>
                            <th class="text-right">{$Yii::t('common','Bank')}</th>
                            <th class="text-right" title="เลขที่บัญชีปลายทาง">{$Yii::t('common','Bank No')}</th>
                            <th class="text-right">{$Yii::t('common','Type')}</th>
                            <th class="text-right">{$Yii::t('common','Remark')}</th>
                        </tr>
                    </thead>`;
        ...
        newData.map(model => {
            ...
            TotalSale   += model.orderTotal;

            let balance     = model.balance;
            let orderTotal  = model.orderTotal;
            let checkClass  = balance.toFixed(2) == orderTotal.toFixed(2) ? 'text-success' : 'text-red' ;
            ...
            html+= `    <td class="pointer invoice-detail text-info `+ ( model.posted == 'Posted' ? '' : 'text-orange')+`"
                                data-id="`+ model.id +`" 
                                data-val="`+ model.no +`" 
                                data-status="`+ model.posted +`"  
                                data-so="` + model.orderNo + `" 
                                data-date="` + model.date + `" `+ ( model.posted == 'Posted' ? '' : 'title="สีส้ม = ยังไม่โพสบิล"')+`
                                data-soid="`+model.orderId+`">` + model.no + `</td>`;
            html+= '    <td>' + model.custName + '</td>';
            html+= '    <td>' + model.saleCode + '</td>';
            html+= '    <td class="text-right ' +(model.discount > 0 ? 'bg-warning' : '')+ '">' + model.discount + '</td>';
            html+= '    <td class="text-right ' +(model.sumline < 0 ? 'text-red' : '')+ '">' + number_format(sumline.toFixed(2)) + '</td>';
            html+= '    <td class="text-right">' + number_format(model.incvat.toFixed(2)) + '</td>';
            html+= '    <td class="text-right" ' +(model.balance < 0 ? 'text-red' : '')+ '>' + number_format(model.balance.toFixed(2)) + '</td>';
            //html+= '    <td>' + model.due + '</td>';
            html+= '    <td><a href="?r=SaleOrders%2Fsaleorder%2Fview&id=' + model.orderId + '" target="_blank">' + model.orderNo + '</a></td>';
            html+= '    <td class="text-right ' + checkClass + '" ' + (checkClass=='text-red' ? 'title="สีแดง=ราคาไม่ตรงกับใบกำกับฯ"' : '' )+ '>' + number_format(orderTotal.toFixed(2)) + '</td>';
            html+= '    <td>' + bankId + '</td>';
        ...
        html+= `<tfoot>
                    <tr class="bg-primary">  
                        ...
                        <th class="text-right">` + TotalDis.toFixed(2) + `</th>
                        <th class="text-right">` + TotalSum.toFixed(2) + `</th>
                        <th class="text-right">` + TotalVat.toFixed(2) + `</th>
                        <th class="text-right">` + grandTotal.toFixed(2) + `</th>                             
                        <th></th>                        
                        <th class="text-right">` + TotalSale.toFixed(2) + `</th>
                        <th></th>
                        <th class="text-right">` + TotalReceipt.toFixed(2) + `</th>
                        <th colspan="4"></th>
                    </tr>
                </tfoot>`;
        html+= `</table>
        ...
        if(res.status===200){                       
            ...
            res.data.raw.map(model => {
                ...
                html+= `<tr data-key="` + model.item + `">
                            ...
                            <td>` + model.name + `</td>    
                            <td colspan="2" class="text-right"><a href="?WarehouseSearch[ItemId]=`+ btoa(model.item) +`&WarehouseSearch[SourceDocNo]=`+so+`&r=warehousemoving%2Fwarehouse" target="_blank" class="text-white" >` + number_format(parseFloat(model.qty)) + `</a></td>                    
