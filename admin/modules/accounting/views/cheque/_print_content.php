...
$table=<<<HTML
    <div style="
        ...
        <div style="border: 0.05em solid #000; width: 20%;  height: {$body->height}; margin: 0 0 0 -0.2mm; float: left;"></div>
...
<table border="0" cellpadding="2" cellspacing="0"  width="100%"  style="width: 100%;">
  <thead >
    <tr >
      ...
      <th align="left" style="width:150px;"><span style="{$showHeader}">เลขที่ใบกำกับ </span></th>
      <th align="center" style="width:100px;"><span style="{$showHeader}">{$Yii::t('common','Date')}</span></th>
      <th align="center" style="width:95px; border:1px"><span style="{$showHeader}">{$Yii::t('common','Due date')}</span></th>
...
HTML;
    ...
    foreach ($data[$i] as $line) {
        ...
        $amount         = $line->balance;
        // $posting_date   = $line->invoice
        //                     ? $line->invoice->posting_date
        //                     : $line->posting_date;

        $d      = date('d', strtotime($line->posting_date));
        $m      = date('m', strtotime($line->posting_date));
        $yTh    = date('Y', strtotime($line->posting_date)) + 543;
        ...
        $Invoice        = $line->inv_total;
        $ivTotal        = $line->balance;
        $Received       = $line->balance_cheque;
        $ivDiscount     = 0;
        ...
        $td.= '<tr>';
            ...
            $td.= '<td valign="top" class="item" align="right">'.number_format($Invoice,2).'</td>';
