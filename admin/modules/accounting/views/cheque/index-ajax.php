...
?>
    ...
    <div class="col-xs-8">
        ...
        <div class="input-group pull-left mr-2" >           
            <?= Html::dropDownList('bank-list-filter', null,
                ArrayHelper::map(
                    ...
                    [
                        ...
                        'options' => [                        
                            @$_GET['bank-list-filter'] => ['selected' => 'selected']
                        ],
                    ]                                     
                ) 
            ?>       
        ...
        <button class="btn btn-info-ew btn-create pull-right"><i class="fa fa-plus"></i> <?=Yii::t('common','Create Doc')?></button>
    </div> 
...
<div class="row mt-5">
    ...
    <div class="col-xs-2 ">
        ...
        <select id="round-digit" class="form-control pull-right hidden-sm hidden-xs hidden-md" style="max-width:70px;">
...
$js =<<<JS
    ...
    const renderTable = (obj) => {
        ...
        data.map((model, key) => {
            ...
            newData.length > 0 
                ? newData.map((e, keys) => { invList+= `<div>` + e.no + ``+((keys+1) < newData.length ? `\n` : `` )+`</div>`})
                : null;
            body+= `
                <tr data-key="`+model.id+`" data-source="`+model.source_id+`">
                    <td>`+(key+1)+` </td>
                    <td class="hidden">`+model.source_id+`</td>
                    <td>`+model.cheque_date+`</td>
                    <td><input type="text" value="`+model.no+`" name="no" class="no-border" style="font-size: 14px; width: 100%;"/></td>
                    <td class="text-right ` + (balance < 0 ? 'text-red' : 'text-green') + `">`+number_format(balance.toFixed(2))+`</td>
                    <td><div class="over-flow-max">`+invList+`</div></td>                    
                    <td class="">`+model.cust_name+`</td>                    
                    <td class="text-right">
                        <button class="btn btn-warning-ew btn-modify-receipt btn-flat btn-sm" ><i class="fa fa-edit"></i></button>
                        ...
                        <button class="btn btn-danger-ew btn-delete-receipt btn-flat btn-sm" ><i class="far fa-trash-alt"></i></button>
                    </td>
                </tr>
            `;
        })
        ...
        let table = `
            <table class="table font-roboto table-bordered table-hover" id="export_index_table">
                <thead>
                    <tr class="bg-gray">
                        ...
                        <th style="width: 125px;">Invoice</th>
                        <th>Customer</th>                        
        ...
        $('body').find('.table-responsive').html(table);
        $("#export_index_table").tableExport({
    ...
    $('body').on('click','.btn-search', function(){
        ...
        let bank    = $('body').find('select[name="bank-list-filter"]').val();
    ...
    const getData = (obj, callback) => {
        fetch("?r=accounting/cheque/get-data", {
            method: "POST",
    ...
    const deleteReceipt = (obj, callback) =>{

        if(confirm("Delete ?")){
            fetch("?r=accounting/cheque/delete-source", {
                method: "POST",
                body: JSON.stringify(obj),
                headers: {
                    "Content-Type": "application/json",
    ...
    $('body').on('click', '.btn-delete-receipt', function(){
        let el = $(this).closest('tr');
        let id = $(this).closest('tr').attr('data-source')
        deleteReceipt({id:id}, res =>{
            if(res.status==200){
                el.hide('show');
                setTimeout(() => {
                    el.remove();
...
?>
<?php $this->registerJsFile('https://cdnjs.cloudflare.com/ajax/libs/TableDnD/0.9.1/jquery.tablednd.js',['depends' => [\yii\web\JqueryAsset::className()]]); ?>  
