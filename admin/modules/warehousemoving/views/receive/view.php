        ...
        <div class="col-sm-12 mb-5 text-right">
            <?php 
                if(isset($_GET['po'])){
                    ...
                    ['/warehousemoving/receive/print','id' => $model->id],['class' => 'btn btn-info mx-2','target'=>'_blank']);

                    
                }

                echo Html::a('<i class="fa fa-print" aria-hidden="true"></i> '.Yii::t('common','Print'),
                    ['print','id' => $model->id],['class' => 'btn btn-info-ew mx-2','target'=>'_blank']);

                echo Html::a('<i class="fa fa-print" aria-hidden="true"></i> '.Yii::t('common','Print Letter'),
                    ['print-letter','id' => $model->id],['class' => 'btn btn-primary mx-2','target'=>'_blank']);
            ?>
                    ...
                    [
                        ...
                        'value' => function($model){                            
                            $doc =  $model->TypeOfDocument != 'Purchase' 
                                        ? $model->order != null 
                                            ? Html::a($model->order->no, ['/SaleOrders/saleorder/view','id' => $model->order->id],['target' => '_blank', 'data-pjax'=>"0"]) 
                                            : $model->SourceDoc
        ...
        <div class="col-md-12">
            ...
            <?php $gridColumns = [
                    ...
                    [
                        ...
                        'value' => function($model) {
                            $html =  Html::a(Html::img($model->itemstb->getPicture(),['class' => 'img-responsive','style' => 'max-width:50px;']), ['/items/items/view-only','id' => $model->items->id],['target' => '_blank', 'data-pjax'=>"0"]);
                            return $html;
                        }
                    ],
                    ...
                    [
                        ...
                        'value' => function($model)
                        {
                            $html =  Html::a((($model->item==1414)
                                                ? $model->ItemNo 
                                                : $model->itemstb->master_code
                                            ), ['/items/items/view-only','id' => $model->items->id],['target' => '_blank', 'data-pjax'=>"0"]);
                            //$html = '<div>'.(($model->item==1414)? $model->ItemNo : $model->itemstb->master_code).'</div>';
                            //$html.= '<div>'.$model->itemstb->barcode.'</div>';
...
<div class="row">
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <h3 class="panel-title"><?=Yii::t('common','Remark')?></h3>
        </div>
