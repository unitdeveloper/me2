...
<table border="0" cellpadding="2" cellspacing="0"  width="100%"  style="width: 100%;">
  <thead>
    <tr>
      ...
      <th style="font-size:12px;" align="right">{$Yii::t('common','Quantity')}</th> 
      <th style="font-size:12px;" align="center">{$Yii::t('common','Measure')}</th> 
      <th style="font-size:12px;" align="center">{$Yii::t('common','Locations')}</th> 
    </tr>
</thead>
<tbody>
HTML;
       foreach ($data[$i] as $line) {
            ...
            if($line->items->id== 1414 && $line->Quantity < 0){
                ...
                $td.= '<tr>';
                    ...
                    $itemCode      =   $line->item ? $line->items->master_code : $line->ItemNo;
