...
const getDataFromAPI = (obj,callback) => {
    fetch("?r=Planning/planning/list-ajax", {
        method: "POST",
    ...
    const changeProirity = () => {
        ...
        fetch("?r=Planning/planning/change-priority", {
            method: "POST",
    ...
    let html = `<table class="table table-bordered table-hover font-roboto" id="export_table">
        ...
        newData.map((model, key) => {
            ...
            html+= `<tr data-key="`+ model.id +`" data-img="`+ model.img +`">
                        ...
                        <td class="item-info item-code" style="min-width: 130px;"><a href="?r=items/items/view&id=`+model.item+`" target="_blank">` + model.code + `</a></td>
                        <td class="item-info item-name" style="min-width: 220px;">` + model.name + `</td>
                        ...
                        <td class="text-right action-minimum_stock" data-val="` + model.minimum_stock + `" style="background-color: #edc0ff;">
                            <input type="text" name="line_minimum" value="` + model.minimum_stock + `" class="line-change form-control  text-right pull-right no-border"  autocomplete="off" style="background-color: #edc0ff; width:100px;" />
...
/*
    $('body').find("#export_table").tableDnD({
        onDrop: function(table, row) {
