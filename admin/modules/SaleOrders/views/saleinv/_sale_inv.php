...
<div class="row">
  <div class="col-sm-12">
    ...
    <?php if($model->status == 'Invoiced'): ?>
    <a href="index.php?r=SaleOrders/saleinv/print&id=<?=$_GET['id']?>" target="_blank" class="btn btn-app btn-info pull-right"><?= Icon::show('print') ?> ใบส่งของชั่วคราว</a>
    <?php endif; ?>

    <?php if($model->status == 'Shiped'): ?>
      <a href="#" class="btn btn-app btn-danger pull-right ew-gen-inv" ew-order-no="<?=$model->no ?>"><?= Icon::show('files-o') ?> เปิดบิล</a> 
    <?php endif; ?>


    <div class="ew-inv-header">
      <div><?=Yii::t('common','Invoice No.')?> : <label>?????????</label></div>
...
<div class="row">
      <div class="col-xs-12 ew-inv-line">
  ...
  <script type="text/javascript">
    $('body').on('click','.ew-gen-inv',function(){
        ...
        route('index.php?r=approval/approve/sale-order','POST',appdata,'ew-inv-line'); 
    });
  </script>
