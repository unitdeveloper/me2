...
$(document).ready(function(){
    ...
    if(data.length > 0){
        ...
        let orders = localStorage.getItem('create-order')? JSON.parse(localStorage.getItem('create-order')) : [];
        if(orders.length > 0){
                        ...
                        let str = $(p).html(); 
                            switch (store) {
                                ...
                                case 4: // HOME HUB
                                    ...
                                    if(str.search("เลขทีเอกสาร") > -1){ // ค้นหา PO
                                        
                                        $(p).first().css('background','pink');
                                        $(p).first().children('b').remove();    // ลบ <b>เลยที่เอกสาร</b> 
                                        let po      = $(p).first().html();           // เอา html ออกมาแยก <br>
                                        let onlyPo  = po.split('<br>');          // แยกอักษรออกจากวันที่ (ขึ้นด้วย <br>)
                                            res     = Object.assign({}, res, { po: onlyPo[0] });
                                    }
...
let renderTable = (data) => {
        ...
        if(model.status===true){  
            ...
            html+= `
                    <tr data-key="`+model.id+`" data-row="`+key+`">
                        ...
                        <td><input type="number" class="form-control text-right"  name="price" value="`+Number(model.price.toFixed(2))+`" autocomplete="off"/></td>
    ...
    $('body').find('#saleheader-ext_document').val(header.po);
     // ถ้ามีวันที่ให้ใส่เข้าไปด้วย
    $('body').find('#saleheader-order_date').val(header.date);
    $('body').find('#saleheader-invoice_no').val(header.inv);
...
localStorage.setItem('order-status',0);
...
$('body').on('click','button.create-sale-line',function(){
    ...
    if(!headers.header.date && headers.header.date===""){
        // ใส่วันที่ก่อน
        ...
        fetch("?r=SaleOrders/wizard/create-sale-line", {
            ...
            .then(response => {
                ...
                // จดจำว่าเคยสร้างแล้ว
                localStorage.setItem('order-status',1);
                localStorage.setItem('create-order',JSON.stringify(response.item));
            })
            .catch(error => {
                console.log(error);
            });
...
$this->render('__script_js_editable');
?>
