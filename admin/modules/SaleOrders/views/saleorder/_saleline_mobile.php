...
<div class="row">
    <div class="col-xs-12">
        <div class="table-responsive no-padding" id="export_table">
        <?php
            $gridColumns = [
                [
                ...
                [
                    ...
                    'headerOptions' => ['class' => 'hidden-xs','style' => 'width:200px;'],
                    'footerOptions' => ['style' => 'min-width:200px;'],
                    'footerOptions' => ['class' => 'hidden-xs-cancel'],
                    'value' => function($model){
                        return Html::a($model->crossreference->no,['/items/items/view', 'id' => $model->item],['target' => '_blank']);
                    },
                ...
                [
                    ...
                    'value' => function($model){
                        ...
                        $InvenByBom     = $model->items->last_possible;
                        //$sumLine        = ($model->quantity * $model->unit_price) - (($model->quantity * $model->unit_price) * ($model->line_discount / 100));
                        ...
                        $html.= '<div class="hidden-sm hidden-md hidden-lg my-10">
                                    <div class="row">
                                        ...
                                        <div class="col-xs-9">
                                            <div class="row">
                                                <div class="col-xs-4 text-right text-show-calulate font-roboto">
                                                    <span class="qty">'.number_format($model->quantity).'</span><span class="text-yellow"> x </span><span class="price">'.number_format($model->unit_price,2).'</span>
                                                </div>
                                                <div class="col-xs-8 text-right text-info total-text-line font-roboto">
                                                    <div class="mb-10 text-show-total">'.number_format($model->sumLine,2).'</div>
                ...
                [
                   ...
                   // 'footer' => '<div class="ew-qty"><input type="number"  step=any  name="InsertQty" id="InsertQty" class="form-control"></div>',
                ],
                ...
                [
                    ...
                    //'footer' => '<div class="ew-price"><input type="number"  step=any   name="InsertPrice" id="InsertPrice" class="form-control"></div>'
                ],
                ...
                [
                   'label' => Yii::t('common','% Discount'),
                    ...
                    'footerOptions' => ['class' => 'hidden-xs'],
                    'value' => function($model){
                        return '<input type="number"  step=any   value="'.($model->line_discount *1).'" class="form-control text-right text-line update-line_discount" name="line_discount">';
                    },
                    //'footer' => 'รวม',
                ...
                [
                    ...
                    'footerOptions' => ['class' => 'hidden-xs'],
                    'value' => function($model){
                        //(quantity * unit_price) - ((line_discount / 100) * (quantity * unit_price)
                        //$total = ($model->quantity * $model->unit_price) - (($model->quantity * $model->unit_price) * ($model->line_discount /100));
            ...
            <?=  GridView::widget([
                ...
                //     ->andwhere(['IN','warehouse_header.status',['Sale']])
                //     ->andWhere(['warehouse_moving.item' => $model->item])
                //     ->andWhere(['warehouse_header.comp_id' => Yii::$app->session->get('Rules')['comp_id']]);
                //     if($query->exists()){
                //         return ['class' => 'danger text-red'];
                //     }else if(\common\models\WarehouseMoving::find()->where(['SourceDoc' => $model->id])->andwhere(['IN','TypeOfDocument',['Sale']])->andWhere(['comp_id' => Yii::$app->session->get('Rules')['comp_id']])->exists()){
...
<div class="item-detail">
    <div class=" ">
        <div class="content">
            <div class="row">
                <div class="col-xs-6"><a href="javascript:void(0);" id="back-btn"><i class="fas fa-arrow-alt-circle-left fa-2x text-black"></i></a></div>
            ...
            <div class="well  margin-top">                
                <div class=" ">
                    ...
                    <div class="row" style="margin-top:5px;">
                        <div class="col-xs-6"><?=Yii::t('common','Discount')?> % </div>
                        <div class="col-xs-6 text-right text-aqua">
                            <input type="number" value="" name="line_discount" step=any   style="font-size:16px;" pattern="^(?:\(\d{3}\)|\d{3})[- . ]?\d{3}[- . ]?\d{4}$"   class="form-control  line-discount text-right update-field "   autocomplete="off"/>
                        </div>
                    </div>  
            ...
            <div class="row" >
                <div class="col-xs-12">
                    <a href="javascript:void(0);" class="delete-btn" style="position:fixed;bottom:10px;"><i class="far fa-trash-alt fa-2x text-danger" title="<?=Yii::t('common','Delete')?>"></i> <?=Yii::t('common','Delete')?></a>
                </div>
            </div>            
        </div>
    </div>
