...
class MallController extends \yii\web\Controller
{
    ...
    public function actionDelete($inv,$order)
    {
        ...
        if(Yii::$app->user->identity->id !==1){
            ...
            return $this->redirect(['/SaleOrders/mall/index']);
        }else{
            ...
            try {
                ...
                $Warehouse = WarehouseHeader::findOne(['SourceDocNo' => $order]);
                if($Warehouse!==null){
    ...
    public function actionCreateSaleLine(){
        ...
        try {
            ...
            return $this->asJson([
                ...
                'item'      => $list,
                'stock'     => self::stockAdjust($inv_header)
            ]);
    ...
    static function stockAdjust($iv){
        $JSON[] = [
            'status' => false,
            'text'      => [
                'message' => 'NULL'
        ...
        $model      = RcInvoiceHeader::findOne($iv->id);
        ...
        if ($model !== null) {
            ...
            if(($Header = \common\models\WarehouseHeader::findOne(['session_id' => $iv->session_id])) ==null){
                $Header     = new \common\models\WarehouseHeader();
            }

            $Header->line_no        = $model->id;
            $Header->PostingDate    = date('Y-m-d',strtotime($iv->order_date)).' '.date('H:i:s');
            $Header->DocumentDate   = date('Y-m-d');
            $Header->TypeOfDocument = "Invoice";
            $Header->SourceDocNo    = $iv->order_id; // Shipment no (Reference from sale_header)
            $Header->DocumentNo     = $GenSeries->GenerateNoseries('Shipment',true);
            $Header->customer_id    = $model->cust_no_;
            $Header->SourceDoc      = $iv->document_no_;
            $Header->Description    = '';
            $Header->Quantity       = 0;
            $Header->address        = '';
            $Header->address2       = '';
            ...
            $Header->remark         = '';
            $Header->comment        = '';
            $Header->session_id     = $iv->session_id;
            if($Header->save()){

                // Delete Line in session
                //WarehouseMoving::deleteAll(['source_id' => $Header->id]);
                ...
                if($query->exists()){
                    ...
                    foreach ($query->all() as $key => $line) {
                        ...
                        $JSON[] = [
                                ...
                                'order'     => $model->no_,
                                'item'      => $line->items->No,
                                'child'     => self::getAssembly($line->items->ProductionBom,0),
                                ...
                                'status'    => true,
                                'production'=> self::producer($line, $Header, $line->quantity),
                                'shipment'  => self::Shipment($line, $Header, $line->quantity),
                                'show_code' => $barcode
                                //'type'      => $line->items->ProductionBom ? 'PROD' : ''
    ...
    static function Output($saleLine,$Header,$qty){
        ...
        $model->qty_before		    = $saleLine->items->liveInven;
    ...
    static function Consumption($Bom,$Header,$output){
        ...
        $model->qty_before		    = $Bom->items->liveInven;
        $model->qty_after		    = $model->qty_before + $model->Quantity;
