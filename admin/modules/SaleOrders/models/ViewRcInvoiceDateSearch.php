 ...
 */
class ViewRcInvoiceDateSearch extends ViewRcInvoice
{
    /**
     * @inheritdoc
     */
    ...
    public function search($params)
    {
        ...
        if (!is_null($this->posting_date) && strpos($this->posting_date, '/') !== false ) {
            ...
            $query->andFilterWhere([
                'DAY(view_rc_invoice.posting_date)'    => ($this->posting_date)? explode('/',$this->posting_date)[0] : null,
                'MONTH(view_rc_invoice.posting_date)'   => ($this->posting_date)? explode('/',$this->posting_date)[1] : null,
                'YEAR(view_rc_invoice.posting_date)'    => ($this->posting_date)? explode('/',$this->posting_date)[2] : null
            ]);

        }else{
            $query->andFilterWhere(['between', 'DATE(posting_date)', Yii::$app->session->get('workyears').'-01-01', Yii::$app->session->get('workyears').'-12-31']);
        } 
