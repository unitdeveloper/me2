...
<?php 
$this->title = Yii::t('common','Customer item sale Show cost');
?>
<div class="row">    
    <div class="col-sm-12">
...
const renderTable = (res) => {
    ...
    let html = `<table class="table table-bordered font-roboto" id="export_table">
                <tbody>
                <tr class="bg-info">
                    <td colspan="7" class="text-right"><h4>{$Yii::t("common","Sum")}</h4></td>
                    <td class="text-right text-green"><h4 class="total-cost"></h4> </td>
                    <td class="text-right text-blue"><h4 class="total-price"></h4></td>                    
                </tr>
                <tr  class="bg-dark">
                    ...
                    <td class="text-right">{$Yii::t("common","Sale Price")}</td>                    
                </tr>`;
        ...
        newData.map(model => {
            ...
            let childTable = `<tr class="bg-warning">
                ...
                detail.map(el => {
                    ...
                    costLine     = (el.qty * el.cost);
                    priceLine    = (el.qty * el.price);
                    sumPriceLine+= priceLine;
                    sumCostLine += costLine;
                    let cost     = (el.cost * 1);
                    let price    = (el.price *1);
                    let overCost = cost > price ? true : false; 
                    childTable  += `<tr data-key="`+ model.item +`">
                                        ...
                                        <td>` + el.unit + `</td>
                                        <td class="text-right ` +(overCost ? 'bg-red' : ' ')+ `" data-tableexport-msonumberformat="\@">` + number_format(parseFloat(cost).toFixed(2)) + `</td>
                                        <td class="text-right ` + (price < 0 ? 'text-red' : '' ) + `"  data-tableexport-msonumberformat="\@">` + number_format(parseFloat(price).toFixed(2)) + `</td>
                                        <td class="text-right ` + (price < 0 ? 'text-red' : '' ) + `" data-tableexport-msonumberformat="\@">` + number_format((el.qty * price).toFixed(2)) + `</td>                                        
                                        <td class="text-right ` +(costLine > priceLine ? 'text-red' : ' ')+ `" data-tableexport-msonumberformat="\@">` + costLine.toFixed(2) + `</td>
                                        <td class="text-right" data-tableexport-msonumberformat="\@">` + priceLine.toFixed(2) + `</td>
                                    </tr>`;
                })
        ...
        html+= `<tfoot>
                    <tr  class="bg-primary">
                        <td colspan="7" class="text-right">{$Yii::t("common","Total")}</td>
                        <td class="text-right">` + number_format(sumCostLine.toFixed(2)) + `</td>
                        <td class="text-right">` + number_format(sumPriceLine.toFixed(2)) + `</td>                    
                    </tr>
                </tfoot>`;
        html+= '</table>';
    ...
    if(res.data.raw.length > 0){
        ...
        $('body').find('.total-price').text(number_format(sumPriceLine.toFixed(2)));
        $('body').find('.total-cost').text(number_format(sumCostLine.toFixed(2)));
