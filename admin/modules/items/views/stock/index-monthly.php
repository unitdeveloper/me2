...
let renders = (recal) => {
    ...
    $.ajax({ 
        url:"index.php?r=items/stock/index-monthly-ajax",
        type: 'POST', 
        data: {recal:recal},
        async:true,
        success:function(getData){
            ...
            setTimeout(() => {
                ...
                $('#export_table .merge-group').each((key,e) => { 
                    $('select[name="groups"]')
                    .append('<option value="'+$(e).data('key')+'" data-text="'+$(e).text()+'" '+(groupId == $(e).data('key') ? 'selected' : null)+'>'+$(e).text()+'</option>')
                });
                $(function() {
...
$(document).ready(function() {
    let eventFilterMonth = () => {
        ...
        $('#export_table .col').map((key,value) => { $(value).hide();  });
        $('#month-filter').val().map((e) => { $('.'+e).show(); }); 
