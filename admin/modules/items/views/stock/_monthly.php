...
$keys       = 'items%2Fstock%2Fmonthly&comp:'.$comp;
...
function getListMenu($header,$i,$ii){
    ...
    if($i == 1){
        ...
        $html.= '       <th style="width:150px;">'.Yii::t('common','Code').'</th>';       
    ...
    foreach ($models as $key => $model) {
        ...
        foreach (ItemsHasGroups::find()->where(['group_id' => $model->id])->all() as $key => $group) {
            ...
            $html.= '           <td>'.$group->items->master_code.'</td>'."\r\n";
            ...
            $itemValues     = $group->items->myItems ? $group->items->myItems->StandardCost * $Dec : $Dec;
            $StandardCost   = $group->items->myItems ? $group->items->myItems->StandardCost : 0;
            ...
            $html.= '           <td class="text-right bg-dark st-1 col" style="font-family: roboto;">'.number_format($StandardCost,2).'</td>'."\r\n";
...
<div class="scroll-div2">
    <table class="table table-bordered table-hover font-roboto" id="export_table" >     
        <?php 
            ...
            if($recal == 1){
                ...
                Yii::$app->cache->set($keys, $html, 60);  
